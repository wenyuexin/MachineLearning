...
> **适合读者**：已了解LLM基础，希望系统掌握UQ/CALibration领域的研究者/工程师

| 你想了解什么 | 跳转章节 |
|-------------|---------|
| LLM不确定性有哪些类型？ | [二、四维不确定性分类法](#二十四维不确定性分类法) |
| 各维度用什么方法量化？ | [三、UQ方法体系](#三uq-方法体系) |
| 如何校准模型置信度？ | [四、校准方法](#四校准方法) |
| 实际应用场景有哪些？ | [五、应用场景](#五应用场景) |

**核心贡献速览**：
1. **四维不确定性分类法**：输入/推理/参数/预测，比传统认知/偶然二分法更全面
2. **推理不确定性被低估**：占多步QA错误的58%，但相关方法最少
3. **方法总览表**：16种方法的维度/效率/访问需求对比
...
| $x$ | LLM 需要回答的问题 |
| $s$ | LLM 生成的回答序列 |
| $w_i$ | 生成序列中的第 $i$ 个 Token |
| $\mathcal{D}$ | LLM 的词汇表/语料库 |
| $U(x)$ | 问题 $x$ 的不确定性 |
| $C(x, s)$ | 给定 $x$ 时生成 $s$ 的置信度 |
| $H(s)$ | 生成序列 $s$ 的熵 |
...
**序列似然**：
$$P(s|x) = \prod_{i=1}^{|s|} P(w_i | x, w_{<i})$$

**预测熵**：
$$H(s|x) = -\sum_{s \in \mathcal{D}} P(s|x) \log P(s|x)$$

**最大Token对数概率**（用于检测幻觉）：
$$\text{Max}(p) = \max_i (-\ln p(w_i))$$

**语义熵**（Semantic Entropy）：
$$H_{\text{semantic}}(s|x) = -\sum_{C \in \mathcal{C}} P(C|x) \log P(C|x)$$

其中 $\mathcal{C}$ 是语义等价聚类，$P(C|x)$ 是该聚类的总概率。
...
| 方法 | 不确定性维度 | 效率特征 | 模型访问 | 是否需额外模型 |
|------|-------------|---------|---------|--------------|
| Input clarification ensembles | 输入不确定性 | 多轮生成 | 黑盒 | 否 |
| ICL-Sample | 输入不确定性 | 多轮生成 | 黑盒 | 否 |
| SPUQ | 输入不确定性 | 多轮生成 | 黑盒 | 是 |
| UAG | 推理不确定性 | 单轮生成 | 白盒 | 否 |
| CoT-UQ | 推理不确定性 | 单轮生成 | 白盒 | 是 |
| TouT | 推理不确定性 | 多轮生成 | 黑盒 | 否 |
| TopologyUQ | 推理不确定性 | 多轮生成 | 黑盒 | 否 |
| SAPLMA | 参数+预测不确定性 | 微调 | 白盒 | 是 |
| Supervised estimation | 参数+预测不确定性 | 微调+多轮 | 白盒 | 是 |
| Semantic Entropy | 预测不确定性 | 多轮生成 | 白盒 | 是 |
| Kernel Language Entropy | 预测不确定性 | 多轮生成 | 白盒 | 是 |
| Ecc(C), Ecc(E), Deg(C), Deg(E) | 预测不确定性 | 多轮生成 | 黑盒 | 是 |
| Eig(C), Eig(E) | 预测不确定性 | 多轮生成 | 黑盒 | 否 |
| MD-UQ | 预测不确定性 | 多轮生成 | 黑盒 | 是 |
| D-UE | 预测不确定性 | 多轮生成 | 黑盒 | 是 |
