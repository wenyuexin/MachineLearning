...
- **Actor（策略网络）**：
  ...
  - **更新方式**：利用Critic提供的TD误差作为优势函数，调整自身网络权重，改变策略梯度，使得高优势动作的概率增加。具体公式为：
    ...
    θ ← θ + α<sub>θ</sub> δ<sub>t</sub> ∇<sub>θ</sub> <i>ln</i> π(a<sub>t</sub> | s<sub>t</sub>; θ)

其中，$ \delta_t $ 为TD误差，$\alpha_\theta$ 为Actor的学习率。
...
2. **Critic评估价值**：Critic计算当前状态的价值 $V(s_t)$ 和下一个状态的价值 $V(s_{t+1})$，并根据及时奖励 $r(t)$，得到TD误差：
   ...
   其中，$\gamma$ 为折扣因子。这个TD误差是一个关于动作 $a$ 好坏的反馈，误差越小，代表该动作的收益就越大，Actor网络应该增大该动作的概率。
...
使用策略梯度方法更新Actor的参数。注意，不同的算法对价值部分有不同的定义，梯度策略中默认使用路径回报$R(\tau)$，DQN使用价值模型来近似动作价值函数$Q_w(s_t, a_t)$，而优势AC算法使用相对优势函数。

$ \nabla_\theta J(\theta) = \log \pi_\theta(a \mid s) \cdot \delta $
