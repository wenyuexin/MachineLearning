...
> Q: Stable Diffusion 为什么选择在潜在空间做扩散，而不是在像素空间？

**参考答案**：  
像素空间维度极高（如 512×512×3），直接做扩散需要大量计算资源和采样步骤。Stable Diffusion 用 VAE 将图像压缩到低维潜在空间（如 64×64×4），在保留语义信息的同时大幅降低了计算量，使得单张图像生成可以在消费级 GPU 上完成，采样速度也显著提升。